...
static int
dynamicStatsEqual(dynstats_bucket_t *pOld, dynstats_bucket_t *pNew) {
	assert(pOld != NULL);
	assert(pNew != NULL);
	return (
		USTR_EQUALS(name) &&
		NUM_EQUALS(resettable) &&
		NUM_EQUALS(maxCardinality) &&
		NUM_EQUALS(unusedMetricLife)
	);
}
...
dynstats_bucket_t *
dynstats_findBucket(rsconf_t *cnf, const uchar* name) {
	dynstats_buckets_t *bkts;
	dynstats_bucket_t *b;
