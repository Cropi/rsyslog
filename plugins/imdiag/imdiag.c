...
/* Module static data */
static tcpsrv_t *pLoadTcpsrv = NULL;  /* our TCP server(listener) TODO: change for multiple instances */
static tcpsrv_t *pRunTcpsrv = NULL;  /* our TCP server(listener) TODO: change for multiple instances */
...
addTCPListener(void __attribute__((unused)) *pVal, uchar *pNewVal)
{
	...
	CHKiRet(tcpsrv.Construct(&pLoadTcpsrv));
	CHKiRet(tcpsrv.SetSessMax(pLoadTcpsrv, iTCPSessMax));
	CHKiRet(tcpsrv.SetCBIsPermittedHost(pLoadTcpsrv, isPermittedHost));
	CHKiRet(tcpsrv.SetCBRcvData(pLoadTcpsrv, doRcvData));
	CHKiRet(tcpsrv.SetCBOpenLstnSocks(pLoadTcpsrv, doOpenLstnSocks));
	CHKiRet(tcpsrv.SetCBOnRegularClose(pLoadTcpsrv, onRegularClose));
	CHKiRet(tcpsrv.SetCBOnErrClose(pLoadTcpsrv, onErrClose));
	CHKiRet(tcpsrv.SetDrvrMode(pLoadTcpsrv, iStrmDrvrMode));
	CHKiRet(tcpsrv.SetOnMsgReceive(pLoadTcpsrv, OnMsgReceived));
	/* now set optional params, but only if they were actually configured */
	if(pszStrmDrvrAuthMode != NULL) {
		CHKiRet(tcpsrv.SetDrvrAuthMode(pLoadTcpsrv, pszStrmDrvrAuthMode));
	}
	if(pPermPeersRoot != NULL) {
		CHKiRet(tcpsrv.SetDrvrPermPeers(pLoadTcpsrv, pPermPeersRoot));
	}

	/* initialized, now add socket */
	CHKiRet(tcpsrv.SetInputName(pLoadTcpsrv, cnf_params, pszInputName == NULL ?
						UCHAR_CONSTANT("imdiag") : pszInputName));
	CHKiRet(tcpsrv.SetOrigin(pLoadTcpsrv, (uchar*)"imdiag"));
	...
	tcpsrv.configureTCPListen(pLoadTcpsrv, cnf_params);
	cnf_params = NULL;
...
BEGINrunInput
CODESTARTrunInput
	CHKiRet(tcpsrv.ConstructFinalize(pRunTcpsrv));
	iRet = tcpsrv.Run(pRunTcpsrv);
finalize_it:
ENDrunInput
...
CODESTARTwillRun
	...
	if(pLoadTcpsrv == NULL || pRunTcpsrv != NULL)
...
CODESTARTmodExit
	if(pRunTcpsrv != NULL)
		iRet = tcpsrv.Destruct(&pRunTcpsrv);
