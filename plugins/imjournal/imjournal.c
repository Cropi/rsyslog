...
handleRotation(struct journalContext_s *journalContext, char* stateFile)
{
	...
	DBGPRINTF("[%s-4] j=%p test=%d\n", __func__, journalContext->j, sd_journal_test_cursor(journalContext->j, journalContext->cursor));
	// char *tmp = NULL;
	// r = sd_journal_get_cursor(journalContext->j, &tmp);
	// DBGPRINTF("[%s-5] r=%d test=%d\n", __func__, r, (journalContext->j && tmp) ? sd_journal_test_cursor(journalContext->j, tmp) : 123456);
	// free(tmp);

finalize_it:
	journalContext->reloaded = 1;
	DBGPRINTF("[%s-6] j=%p reloaded=%d atHead=%d cursor=%s iRet=%d\n", __func__, journalContext->j, journalContext->reloaded, journalContext->atHead, journalContext->cursor, iRet);
	RETiRet;
}
	...
	if ((r_sf = fopen(stateFile, "rb")) != NULL) {
		...
		if (fscanf(r_sf, "%128s\n", readCursor) != EOF) {
			...
			if (r != 0) {
				...
				DBGPRINTF("[%s-3] r=%d test=%d\n", __func__, r, sd_journal_test_cursor(journalContext->j, readCursor));
				/*
